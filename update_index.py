...
def generate_index(reports):
    ...
    cards_html = ""
    for r in reports:
        cards_html += f'''    <a class="report-card" href="reports/{r['filename']}">
      <span class="report-date">{r['date']}</span>
      <span class="report-title">{r['title']}</span>
      <span class="report-tag">Claude</span>
    </a>\n'''

    empty_msg = '    <div class="empty">尚無報告，請新增第一份報告。</div>\n' if not cards_html else cards_html
...
<body>
  <header>
    ...
    <div class="stats">
      <span class="stat">共 {{count}} 份報告</span>
...
def main():
    ...
    print(f"✅ index.html 已更新，共 {len(reports)} 份報告")
